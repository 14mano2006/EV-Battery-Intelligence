...
import { BatteryPack3D } from "./components/BatteryPack3D";
...
function getPageFromPath() {
  ...
  if (path.includes("/pack") || path.includes("/cargo") || path.includes("/3d")) {
    return "pack";
  }

  if (path.includes("/intelligence") || path.includes("/fleet") || path.includes("/realtime") || path.includes("/telemetry") || path.includes("/analytics") || path.includes("/prediction")) {
    return "intelligence";
  }
...
function navigateToPage(page) {
  setTimeout(() => {
    let url = "/";
    if (page === "map") url = "/charging-map";
    else if (page === "pack") url = "/cargo-layout";
    else if (page === "intelligence" || page === "realtime" || page === "fleet" || page === "analytics" || page === "prediction") url = "/intelligence";
    else if (page === "maintenance") url = "/maintenance";
    else if (page === "addVehicle") url = "/add-vehicle";
...
function App() {
  ...
  const [loadingRul, setLoadingRul] = useState(false);
  ...
  useEffect(() => {
    ...
    const loadRul = async () => {
      try {
        setLoadingRul(true);
        const response = await fetch(`${API_URL}/vehicles/${selectedVehicle}/rul`);
        if (!response.ok) {
          throw new Error("Unable to load RUL data.");
        }
        const data = await response.json();
        setRul(data);
      } catch (err) {
        console.error("RUL loading error:", err);
        setRul(null);
      } finally {
        setLoadingRul(false);
      }
  ...
  const handleAddVehicle = async (event) => {
    ...
    try {
      ...
      if (vehiclesResponse.ok) {
        ...
        const updatedVehicles = Array.isArray(vehiclesData) ? vehiclesData : (vehiclesData.vehicles || []);
                    ...
                    {maintenanceVehicles.map(
                      (vehicle) => (
                        ...
                        >
                          ...
                          <td>
                            {vehicle.remaining_useful_life_cycles != null ? (
                              <div style={{ display: "flex", flexDirection: "column" }}>
                                <strong style={{
                                  color: vehicle.remaining_useful_life_cycles <= 50 ? "#ef4444" :
                                         vehicle.remaining_useful_life_cycles <= 120 ? "#f59e0b" : "#10b981",
                                  fontSize: "12px",
                                  fontFamily: "monospace"
                                }}>
                                  {vehicle.remaining_useful_life_cycles} cyc
                                </strong>
                                <small style={{ color: "#94a3b8", fontSize: "10px" }}>
                                  {vehicle.estimated_remaining_km ? `~${Math.round(vehicle.estimated_remaining_km / 1000)}k km` : "EOL 70%"}
                                </small>
                              </div>
                            ) : (
                              "--"
  ...
  return (
    <div className="min-h-screen bg-[#07090e] text-slate-100 flex flex-col font-sans selection:bg-amber-500 selection:text-black">
      {/* TOP TACTICAL COCKPIT HEADER WITH LIVE FLEET TICKER */}
      <CockpitHeader
        vehicles={vehicles}
        selectedVehicle={selectedVehicle}
        onSelectVehicle={(vId) => setSelectedVehicle(vId)}
        searchQuery={searchQuery}
        onSearchChange={setSearchQuery}
        alertCount={maintenance?.total_high_priority ?? 2}
        onOpenAlerts={() => {
      ...
      {/* MAIN VIEWPORT */}
      <main className="flex-1 md:ml-20 p-4 sm:p-6 max-w-7xl w-full mx-auto pb-20">

        <AnimatePresence mode="wait">
          <motion.div
            key={activePage + (selectedVehicle || "")}
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            transition={{ duration: 0.2, ease: "easeOut" }}
          >
            {activePage === "intelligence" || activePage === "realtime" || activePage === "fleet" || activePage === "analytics" || activePage === "prediction" ? (
              ...
              <FleetIntelligence
                ...
                animatedPrediction={animatedPrediction}
